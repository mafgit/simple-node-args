...
describe('check_if_flag', () => {
  test('should pass, passed short flag, flag_options must match, is_flag must be true', () => {
    const arg = '-n'
    check_if_flag(flag_schemas, arg, (err, is_flag, flag_options) => {
      expect(err).toBeNull()
      expect(is_flag).toBe(true)
      expect(flag_options).toEqual(flag_schemas.find((i) => i.short === 'n'))
    })
  })
  ...
  test('should pass, passed long flag, flag_options must match, is_flag must be true', () => {
    ...
    check_if_flag(flag_schemas, arg, (err, is_flag, flag_options) => {
      expect(err).toBeNull()
      expect(is_flag).toBe(true)
      expect(flag_options).toEqual(flag_schemas.find((i) => i.long === 'git'))
    })
  })
